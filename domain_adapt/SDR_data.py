...
>>>>>>> .merge_file_yH3jQd
    ...
    if u_remaining_dim > 0:
        p_u_remaining_dist = scipy.stats.multivariate_normal(mean=np.zeros(shape=u_remaining_dim), cov=np.eye(u_remaining_dim)*remaining_noise)
    ...
    print v1_constant_right_axis_boundaries, 'v1_constant_right_axis_boundaries'
    print v1_constant_left_axis_boundaries, 'v1_constant_left_axis_boundaries'
#    pdb.set_trace()
...
#        fig.tight_layout()
    ...
    @classmethod
    def plot_us(cls, B, xs_train, xs_test, ax=None):
        fig = None
        if ax is None:
            fig, ax = plt.subplots()
        ax.set_xlabel('B * x')
        s = 1.
        ax.scatter(np.dot(xs_train, B), np.zeros(len(xs_train))-1, color=cls.train_color, label='train', s=s)
        ax.scatter(np.dot(xs_test, B), np.zeros(len(xs_test))-1, color=cls.test_color, label='test', s=s)
        alpha = 0.5
        bins = 'auto'
        ax.hist(np.dot(xs_train, B), color=cls.train_color, alpha=alpha, bins=bins)
        ax.hist(np.dot(xs_test, B), color=cls.test_color, alpha=alpha, bins=bins)
        ax.legend()
        if not fig is None:
            fig.tight_layout()
        if not (fig is None):
